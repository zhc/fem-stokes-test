...
def collect_square_result(element, h, u, p, time, mesh):
    collect_u_p_result(element, h, u, p, time, mesh, "square")
    square_time_collector.append( {"h":h, "e":element, "t":time} )
def collect_square_error(element, h, u_err, p_err):
    square_err_collector.append( {"h":h, "e":element, "u_err":u_err, "p_err":p_err} )
